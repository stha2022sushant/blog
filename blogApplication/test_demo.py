...
# Import the BlogFactory to generate blog data
...
class TestBlogMutation(TestCase):
    def setUp(self):
        ...
        # # This will create a blog with random title,
        # content, and optional author
    ...
    def test_create_blog(self):
        ...
        if data and data.get('createBlog'):
            ...
            self.assertEqual(str(created_blog_id), str(self.blog.id))
            # Compare the dynamic ID
        else:
            # Check for errors in the response if 'createBlog' is not present
            errors = response_data.get('errors')
            if errors:
                print(errors)  # Print the errors for debugging
            self.fail("Blog creation failed,
            'createBlog' is not in the response.")
